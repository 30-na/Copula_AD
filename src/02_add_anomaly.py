"""Add a controlled anomaly to one-dimensional simulated data and plot the result.
...
Outputs:
  - data/simulation/simulated_with_anomaly.csv
  - data/simulation/simulated_anomaly_labels.csv
  - results/simulation/figures/simulated_with_anomaly.png
"""
...
def add_variance_burst(
    ...
    """Increase noise variance over a known interval."""
    if start_index < 0:
    ...
    baseline_std = float(df[value_column].std())
    burst_noise = rng.normal(loc=0.0, scale=strength * baseline_std, size=duration)
    output.iloc[start_index : start_index + duration, 0] += burst_noise
...
def plot_anomaly(
    ...
    ax.plot(anomaly_df.index, anomaly_df.iloc[:, 0], color="black", linewidth=0.8)
    ax.axvspan(start_time, end_time, color="red", alpha=0.15, label="anomaly")
    ax.set_title("Simulated Time Series With Controlled Variance-Burst Anomaly")
    ax.set_xlabel("Time")
    ax.set_ylabel("value")
...
def main() -> None:
    ...
    parser.add_argument("--input", default="data/simulation/simulated_clean.csv")
    parser.add_argument("--output", default="data/simulation/simulated_with_anomaly.csv")
    parser.add_argument("--labels-output", default="data/simulation/simulated_anomaly_labels.csv")
    parser.add_argument("--plot", default="results/simulation/figures/simulated_with_anomaly.png")
    ...
    plot_anomaly(anomaly_df, labels, Path(args.plot))
